...
When committing a passed task, follow these steps:
...
1. Locate the folder `.tequila/tasks/{task-id}/` with the specified `task-id`.
   ...
   - If the task is not in the `PASSED` state, inform the user and abort the current action.
